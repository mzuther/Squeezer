...
\begin{itemize}
\item use output gain to match the loudness of compressed and direct
  signal before comparing them\footnote{if you don't do this, the
    louder signal will automatically sound better}

\item set the threshold to a fixed value and control Squeezer
  \emph{vintage style} by changing trim and output gain only

\item if Squeezer sounds well and the level of the incoming signal
  changes for some reason, I find it easier to adjust using trim
