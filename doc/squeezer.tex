...
\emph{I wrote this compressor\footnote{In this manual, I will use
    ...
    There are other ways to reduce dynamic range, but in professional
    audio, downward compression is used almost exclusively.} to learn
  all I could about audio compression.  It took me several months to
  gather all the specs and papers I needed and convert them into
  code.}
...
\emph{Compressors are inherently complex sound processors.  The
  technology of compressors is quite simple -- their complexity arises
  from the fact that most controls interact with many other controls.
  While this is definitely a downside when you try to understand
  compression, it is also the very thing that makes compressors such
  creative devices!}
...
\begin{tabular}{p{0.3 \textwidth}p{0.625 \textwidth}}
  ...
  measures level of audio input and sends result (\SI{-27.3957}{\dBFS})
  to gain computer \\

  \textbf{gain computer} &
  compares input level to desired output level and sends calculated
  gain reduction (\SI{-3.1412}{\dB}) to smoothing filter \\

  \textbf{smoothing filter} &
  smoothly adjusts current gain reduction in the direction of
  calculated gain reduction; uses two different speeds (attack and
  release) depending on direction \\
...
As its name implies, the level detector identifies an audio signal's
level.  It can either detect the input wave form directly (peak
sensing) or average the input signal (usually by passing the input
signal through a RMS\footnote{root mean square} filter).

Peak sensing allows a compressor to react fast to sudden changes in
level, whereas RMS sensing detects changes in the audio material's
\emph{loudness} which often yields very even-sounding results.

There are two ways of connecting a level detector: the old way is to
detect the gain stage's output level (feed-back design), leading to a
very distinct ``bouncing'' sound.  The modern way is to detect the
compressor's input signal directly (feed-forward design), yielding a
more natural sound.

\subsection{Gain computer}
\label{sec:gain_computer}

The gain computer\footnote{in the sense of ``calculator'', can consist
  entirely of analogue circuitry} is fed by the level detector and
converts a given input level to an output level by means of a transfer
curve.  It then uses these levels to calculate a corresponding gain
reduction (``attenuate the current input signal by \SI{2.5}{\dB}'').
This gain reduction changes \emph{constantly} and in tune with the
input signal.

The most common gain computer design is a threshold control.  It sets
a level below which all input passes the gain computer unchanged.
Levels passing the threshold are attenuated by a compression ratio,
set through a second control.  Higher compression ratios effect more
compression when compared to lower ratios.  Note that a compression
ratio of 1:1 effectively bypasses the compressor.

Looking for an example?  A threshold of \SI{-20}{\dBFS} yields a gain
reduction of \SI{0}{\dB} (no compression) for any level below
...
There may be a third control called knee width.  It defines a
transition zone around the threshold where the compression ratio
gradually changes from 1:1 to the selected ratio.  Quiet signals will
receive less compression than loud signals, effectively reducing the
audible artefacts caused by compression.
