...
reduction changes \emph{constantly} over time.
...
compression and I \emph{do} like the sound -- but it's not something
you want to have on every track, let alone a full song.
...
To change \emph{sounds} instead of \emph{single wave forms}, gain
reduction has to be slowed down.  One possibility is using averaging
level detectors\footnote{this is \emph{not} the reason why compressors
  have averaging level detectors}, employing photo cells is
...
Both solutions effectively slow down the compressor's response time.
However, we need more control to accommodate to different types of
sound.  This is where time constants (attack and release rate) come
in.
