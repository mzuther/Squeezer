...
These knobs control both \textbf{threshold} and compression ratio.
...
\textbf{Note: upward expansion \emph{increases} the output level, so
  take care of you ears -- especially when you switch from compression
  to expansion!}
...
These knobs let you change Squeezer's response time.
...
\textbf{Trim} allows you to simultaneously change Squeezer's input
gain and output gain in opposite directions.

In additional, you can control \textbf{output} gain independently.  I
can think of several use cases:

\begin{itemize}
\item use \emph{output gain} to match the loudness of compressed and
  direct signal before comparing them\footnote{the brain perceives
    loud signals to sound better than quiet ones}
...
\item if Squeezer sounds well and the level of the incoming signal
  changes for some reason, I find it easier to adjust using
  \emph{trim} instead of \emph{threshold}

\item use gain staging to improve automatic make-up gain
  (\ref{sec:buttons_automatic_makeup_gain}) using \emph{trim}
\end{itemize}
  
\section{Stereo link and wet/dry ratio}
...
of compressed and direct signal, whereas \SI{0}{\percent} effectively
bypasses Squeezer.

Mixing is latency-compensated, so you can use the wet control to
easily set-up \emph{parallel compression}.
...
filter.  These knobs control their cutoff frequencies.

Both filters are connected in series so you can employ them
simultaneously to leave only a specific band of frequencies.
...
Engaging the feed-back button places the level detector at the gain
stage's output.  Otherwise, the detector is fed with the unprocessed
input signal (feed-forward compression).
...
Using this button, you can feed Squeezer's side-chain with an external
signal.  Please see \ref{sec:external_side_chain} for more
information.
...
By clicking on this button, you can listen to the side-chain (internal
or external) including filtering.  \emph{Note: do not forget to
  disable this button when you are done \dots}
...
When this button is engaged, Squeezer tries to counter level jumps
caused by changing threshold and compression ratio.

Implementing automatic make-up gain is an exercise in compromise, so
its quality depends on the incoming signal's level (and a lot of other
factors).  Changing input trim (\ref{sec:input_trim_and_output_gain})
can sometimes improve results.
