...
directly, you'd have a wave shaping\footnote{\emph{wave shaping} is a
  fancy way of saying \emph{distortion}} device.  By reacting
...
not something you want to have on every track, let alone a full song.
...
employing photo cells is another.\footnote{photo cells react to level
  changes in complex time-dependant ways} Both solutions effectively
slow down the compressor's response time, but somewhat crudely.
...
slowly.\footnote{incidentally, that is \emph{exactly} how vintage
  photo cells react to changes} Also, a single ``speed'' of smoothing
...
two-thirds of an \emph{downward} change in gain
...
using the attack time.  Even when the slope starts to drop,
...
smoothed according to the release time.  This continues until
\emph{current} gain reduction starts to drop again and falls below
\emph{target} gain reduction, when the attack time will once again
take over.
...
understanding their dynamic state.  I still regard the above necessary
knowledge, however, because it will let you \emph{disregard} the
countless erroneous explanations out there.\footnote{which were the
  reason for developing Squeezer in the first place} In my opinion,
the erroneous explanations make it almost \emph{impossible} to
...
Vintage compressors use simple analogue filters with logarithmic
curves.  Speed depends on the difference between current and target
gain reduction -- initial change is fast and slows down as the
difference gets smaller.

Newer compressors often have linear curves -- the target gain
reduction is approached with constant speed.

A third curve is often called ``S-curve''.  In the case of Squeezer,
attack and release time behave like logarithmic curves.  However, when
the attack phase changes to release, a smooth transition between the
curves prevents a sharp drop in gain reduction.  This reduces yet
another type of distortion exhibited by compressors.
...
The gain stage is an amplifier that attenuates the input signal by the
current gain reduction.  There may be an additional gain control to
make up for any level lost during compression (``make-up gain'').

That's it.

% TODO: stereo link, parallel compression, side-chain
% filtering, external side-chain
...
\emph{Sorry, I have yet to find the time to finish this manual.}
