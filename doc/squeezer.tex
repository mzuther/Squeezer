...
\begin{tabular}{p{0.3 \textwidth}p{0.625 \textwidth}}
  ...
  and sends resulting gain reduction to smoothing filter \\

  \textbf{Smoothing filter} &
  smoothly adjusts \emph{current} gain reduction in direction of
  \emph{calculated} gain reduction and sends result to gain stage;
  the ``speed'' of smoothing can often be adjusted (\emph{time
    constants}) \\
...
ratio (full compression).  Thus, quiet signals receive less
compression than loud signals and the compressor sounds ``smoother''.
...
adjusted in the direction of the \emph{calculated} gain reduction.
...
\item compression occurs \textbf{whenever \emph{current} and
    \emph{calculated} gain reduction differ}; this means \textbf{all
    of the time the detected level is above threshold} and slightly
  after
\end{itemize}
...
will fall below \emph{calculated} gain reduction and will be smoothed
using the attack time.  Even when the slope starts to drop,
\emph{current} gain reduction may still be below \emph{calculated}
gain reduction and will be affected by the attack time.

If the slope keeps falling, \emph{current} gain reduction will
eventually rise above \emph{calculated} gain reduction and will be
...
\emph{calculated} gain reduction, when the attack time will once again
take over.
...
threshold.  And this is what makes them so hard to explain.
