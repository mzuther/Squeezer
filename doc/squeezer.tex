...
\emph{Compressors are inherently complex sound processors.  Their
  ...
  interaction of some controls.  While this is definitely a downside
...
range\footnote{difference between loudest and quietest level} of audio
...
\begin{tabular}{p{0.3 \textwidth}p{0.625 \textwidth}}
  ...
  measures level of audio at input and sends result to gain computer
  (for instance \SI{-27.39}{\dBFS}) \\

  \textbf{Gain computer} &
  calculates difference between input level and desired output level
  and sends result to smoothing filter (gain reduction, for example
  \SI{-3.14}{\dB}) \\

  \textbf{Smoothing filter} &
  smoothly adjusts \emph{current} gain reduction in direction of
  \emph{calculated} gain reduction; uses two different speeds
  (attack and release rate) depending on direction of change \\
...
sensing) or average the input signal (usually with a RMS\footnote{root
  mean square} filter).

\emph{Peak sensing} allows a compressor to quickly react to sudden
changes in level, whereas \emph{RMS sensing} detects changes in the
audio material's \emph{loudness} which often yields very even-sounding
results.
...
design), resulting in a more natural sound.

\section{Gain computer}
\label{sec:gain_computer}

The level detector's signal is sent to a gain computer\footnote{in the
  sense of ``calculator''; some gain computers are entirely built from
  analogue circuitry} which determines the output level the signal
should have.  It then calculates a corresponding gain reduction
(``attenuate the current input signal by \SI{3.14}{\dB}'').  This gain
reduction changes \emph{constantly} and in tune with the input signal.

The most common gain computer design is a \emph{threshold} control.
It sets a level below which all input passes the gain computer
unchanged.  Levels exceeding the threshold are attenuated by a
\emph{compression ratio}, set through a second control.  Higher ratios
effect more compression than lower ratios.  Note that a compression
ratio of 1:1 effectively bypasses the compressor.

Looking for an example?  A threshold of \SI{-20}{\dBFS} yields a gain
reduction of \SI{0}{\dB} (no compression) for all levels below
\SI{-20}{\dBFS}.  A signal of \SI{-19}{\dBFS} will produce a gain
reduction of \SI{-0.5}{\dB} for a compression ratio of 2:1 and
\SI{-0.25}{\dB} for a ratio of 4:1.
...
ratio gradually changes from 1:1 to the selected ratio.  This means
that quiet signals will receive less compression than loud signals,
effectively reducing the audible artefacts caused by compression.

\section{Smoothing filter}
\label{sec:smoothing_filter}

\emph{So far, so good.  This was the easy part, but now comes the part
  where most explanations start to fail.  I'll try my best and
  continue anyway \dots}

If you stopped here and let the gain computer control the gain stage
directly, you'd have a wave shaping\footnote{``wave shaping'' is a
  fancy way of saying ``distortion''} device.  The compressor would
react so fast that it relentlessly changed the shape of \emph{single
  wave forms}.

Don't get me wrong: this already \emph{is} compression and I \emph{do}
like the sound -- but it's not something you want to have on every
track, let alone a full song.

\emph{Listen for yourself!  Load a drum track into your DAW, add the
  Squeezer plug-in, unlock attack and release rate\footnote{click on
    the red lights next to the knobs so they light up} and set them to
  their lowest value.  Make sure the compressor is set to a high
  compression ratio and uses peak sensing\footnote{``RMS'' button is
    not engaged}.  Now lower the threshold until you can clearly hear
  the wave shaping.  But don't close the DAW yet \dots}

What's the problem here?  Compression currently happens too quickly.
We want to change \emph{sounds}, not \emph{single wave forms}.  So we
have to slow down gain reduction.
...
One possibility is using averaging level detectors\footnote{this is
  ...
  detectors}, employing photo cells is another\footnote{photo cells
  react to level changes in complex time-dependant ways}.  Both
solutions effectively slow down the compressor's response time, albeit
crudely.  As there are many different types of sound, however, some
more control would be welcome.  This is where time constants (attack
and release rate) come in.

\emph{Play the drum track and engage the ``RMS'' button -- the
  distortion vanishes.  Disengage the ``RMS'' button and enable the
  ``Opto'' button instead -- the distortion vanishes, but the track
  sounds slightly different.  You can now close the DAW.}
...
\emph{Sorry, I have yet to find enough time to write this manual.}
