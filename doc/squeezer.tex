...
that his drinks sold best when the music fell into a certain loudness
range.  So he bought a loudness meter and wrote a set of instructions
for the DJ:
...
getting a \emph{stereo} mixer and the new counter didn't go down too
well with him).  After an initial shock, the bar keeper was quite
content.  The DJ really \emph{had} been quite lousy.

Being an inventor, the bar keeper also opened the mixer and connected
its output to a simple amplifier.  The amplifier's gain was regulated
by a level sensor which in turn was connected to the amplifier's
output.

Although the new device exactly followed the DJ's instructions (and
didn't smell as bad), customers complained about its sound quality.
The bar keeper had invented a \emph{wave shaping} device -- gain
changes were applied instantly which \emph{distorted} its input
signal.
...
bar keeper improved his device by adding a smoothing device between
level sensor and amplifier.
